...
def download_files_cve(import_path, nvd_config: NVDSourceConfig | None = None):
    ...
    logger.info('Updating the Database with the latest CVE Files...')
...
def download_files_cpe(import_path, nvd_config: NVDSourceConfig | None = None):
    ...
    if config.source != "api":
        logger.warning("CPE mirror feeds are unavailable. Provide NVD_API_KEY and set NVD_SOURCE=api to import CPE data.")
        return
    ...
    logger.info('Updating the Database with the latest CPE Files via NVD API...')
    client = NVDApiClient(config.api_key)
...
def download_files_cwe(import_path):
    ...
    download_folder = os.path.join(import_path, "mitre_cwe")
    extract_dir = download_folder

    # Download and Unzip the files
    logger.info('Updating the Database with the latest CWE Files...')
...
def download_files_capec(import_path):
    ...
    download_folder = os.path.join(import_path, "mitre_capec")
    extract_dir = download_folder

    # Download xml file
    logger.info('Updating the Database with the latest CAPEC Files...')
...
def download_datasets(import_path, nvd_config: NVDSourceConfig | None = None, skip_cpe: bool = False):
    config = nvd_config or NVDSourceConfig.from_env()
...
def make_http_request_with_retry(url, retries=0):
    try:
        ...
        if retries < MAX_RETRIES:
            logger.warning(f"Circuit is open. Retrying... Attempt {retries + 1}")
            return make_http_request_with_retry(url, retries=retries + 1)
        else:
            raise RuntimeError("Circuit is open. Max retries reached.")
    except Exception as e:
        if retries < MAX_RETRIES:
            logger.warning(f"Error occurred: {e}. Retrying... Attempt {retries + 1}")
            return make_http_request_with_retry(url, retries=retries + 1)
        else:
...
def unzip_files_to_directory(zip_path, extract_path, zip_filename):
    try:
        ...
        logger.info(f"{zip_filename} unzipped successfully")
        logger.debug('---------')
        z.close()
        file_to_delete = os.path.join(extract_path, zip_filename)
        if os.path.exists(file_to_delete):
            os.remove(file_to_delete)
    except zipfile.BadZipfile as e:
        logger.error(f"Error while unzipping data: {e}")

def transform_xml_files_to_json(path):
    directory_contents = os.listdir(path)

    for item in tqdm(directory_contents, desc="XML→JSON", unit="file", leave=False):
...
def transform_big_json_files_to_multiple_json_files(path, output_prefix, json_array_path):
    ...
    for item in tqdm(directory_contents, desc=f"{output_prefix} files", unit="file", leave=False):
        item_path = os.path.join(path, item)
...
def xml_file_to_json(xmlFile):
    ...
    logger.info(f"Transforming file {xmlFile}")
...
def replace_unwanted_string_cwe(path):
    ...
    fin = open(file, "rt")
    flattened_cwe = os.path.join(path, "cwe.json")
...
def replace_unwanted_string_capec(path):
    ...
    file = os.path.join(path, files[0])
    fin = open(file, "rt")
    flattened_cwe = os.path.join(path, "capec.json")
    fout = open(flattened_cwe, "wt")
    for line in fin:
...
def slice_json_file(input_file, output_path, output_prefix, batch_size, json_array_path):
    ...
    splitted_path = os.path.join(output_path, "splitted")
    if not os.path.exists(splitted_path):
        os.makedirs(splitted_path, exist_ok=True)

    for batch_index, start in enumerate(tqdm(range(0, length, batch_size), desc=f"{output_prefix} batches", unit="batch")):
        batch = data_array[start:start+batch_size]
        output_file = os.path.join(splitted_path, f"{output_prefix}_output_file_{batch_index + 1}.json")
...
def run_dos2unix(file):
    logger.info(f"Normalizing line endings for {file}")
    ...
    if dos2unix_binary:
        try:
            ...
            if process.returncode == 0:
                logger.info(f"File {file} transformed to unix format")
            else:
    ...
    try:
        ...
        with open(file, "wb") as f:
            f.write(normalized)
        logger.info(f"File {file} normalized without dos2unix binary")
